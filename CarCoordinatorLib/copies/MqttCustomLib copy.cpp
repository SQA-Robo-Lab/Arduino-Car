...
static void consumeMessageIntoBuffer(char* topic, byte* payload, unsigned int length){
    ...
    ArduinoMessageBuffer* bufferToFill = NULL;
    while(*list != NULL){ //iterate through the list
        ...
        if (strcmp(topic, currentSubscriber->topic) == 0){
            bufferToFill = currentSubscriber->messageBuffer;
            break;
        }
        list = &(*list)->nextElement;
    }
    if (bufferToFill != NULL){
        ArduinoMessageBuffer_push(bufferToFill, payload);
    }
}
...
void createAndRegisterMqttSubscriber(MqttSubscriber* subscriber,
                                    ...
                                    size_t messageSize,
                                    bool_t bufferMode){
    subscriber->messageBuffer = ArduinoMessageBuffer_create(bufferCapacity, messageSize, bufferMode);
